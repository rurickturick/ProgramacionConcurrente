...
// ------------------------------- Parte modificada para el apartado 1 ---------------------------------------
  ...
  public  void realizar_reintegro(Cuenta cta, int cantidad, Cliente reintegrador) {
	  synchronized(cta){  
		  while((cta.saldo(this)-cantidad)<0){			  
			  try {
				System.out.println("El cliente "+reintegrador.id+" está a la espera de saldo suficiente en la cuenta #" +
						cta.numCuenta(this)+" con cajero "+this.id);
				cta.wait();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		  }
		  System.out.println("El cliente "+reintegrador.id+" puede hacer el reintegro" +
				  " en la cuenta #" +cta.numCuenta(this)+" con cajero "+this.id);
		  cta.actualizarSaldo(this, cta.saldo(this) - cantidad);
	  } 
   }   
// ------------------------------------------------------------------------------------------------------------
